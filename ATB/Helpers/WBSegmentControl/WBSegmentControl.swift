...
//                let font = segmentTextBold ? UIFont.boldSystemFont(ofSize: self.segmentTextFontSize) : UIFont.systemFont(ofSize: self.segmentTextFontSize)
                // added this
                let font = (index == self.selectedIndex) ? UIFont(name: "SegoeUI-Bold", size: self.segmentTextFontSize)! : UIFont(name: "SegoeUI-Light", size: self.segmentTextFontSize)!
    ...
    func didSelectedIndexChanged(_ newIndex: Int, oldIndex: Int) {
        ...
        if validIndex(oldIndex) {
            ...
            case .text:
                if let old_contentLayer = self.innerSegments[oldIndex].layerText as? CATextLayer {
                    ...
                    // added for UI matching in the application
                    old_contentLayer.font = CGFont(NSString(string: UIFont(name: "SegoeUI-Light", size: self.segmentTextFontSize)!.fontName))
                }
            default:
                break
            }
        ...
        case .text:
            if let new_contentLayer = self.innerSegments[newIndex].layerText as? CATextLayer {
                ...
                // added for UI matching in the application
                new_contentLayer.font = CGFont(NSString(string: UIFont(name: "SegoeUI-Bold", size: self.segmentTextFontSize)!.fontName))
            }
        default:
            break
        }
...
//                ])
            ...
            size = (text as NSString).size(withAttributes: [
                NSAttributedString.Key.font: segmentTextBold ? UIFont(name: "SegoeUI-Bold", size: self.segmentTextFontSize)! : UIFont(name: "SegoeUI-Light", size: self.segmentTextFontSize)!
            ])
        case let .icon(icon):
            size = icon.size
        }
