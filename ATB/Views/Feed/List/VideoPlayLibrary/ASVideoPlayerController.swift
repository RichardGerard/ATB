...
//  Updated by YueXi on 03/22/2021
...
class ASVideoPlayerController: NSObject, NSCacheDelegate {
     ...
     */
    func setupVideoFor(url: String, index: Int) {
        ...
        asset.loadValuesAsynchronously(forKeys: requestedKeys) { [weak self] in
            ...
            DispatchQueue.main.async {
                ...
                if strongSelf.videoURL == url, let layer = strongSelf.currentLayer {
                    strongSelf.playVideo(withLayer: layer, url: url,index: index)
                }
            }
        }
    }
    // Play video with the AVPlayerLayer provided
    func playVideo(withLayer layer: AVPlayerLayer, url: String,index:Int) {
        videoURL = url
        currentLayer = layer
        if let videoContainer = self.videoCache.object(forKey: url as NSString) {
            ...
            addObservers(url: url, videoContainer: videoContainer,index: index)
        }
    ...
    private func addObservers(url: String, videoContainer: ASVideoContainer,index:Int) {
        ...
        if self.observingURLs[url] == false || self.observingURLs[url] == nil {
            ...
            let interval = CMTime(seconds: 4,
                                  preferredTimescale: CMTimeScale(NSEC_PER_SEC))
            let cmtimevalue = NSValue(time: interval)
            let timeObserverToken = videoContainer.player.addBoundaryTimeObserver(forTimes: [cmtimevalue], queue: nil) {
                print(videoContainer.player.currentTime().seconds)
                if index >= 0{
                    let noti = index
                    NotificationCenter.default.post(name: NSNotification.Name("PlayBack"), object: noti, userInfo: nil)
                }
                
//                videoContainer.player.removeTimeObserver(timeObserverToken)
            }
    ...
    func pausePlayeVideosFor(tableView: UITableView, appEnteredFromBackground: Bool = false) {
        ...
        if maxHeight > minimumVideoLayerVisibleHeight {
            if appEnteredFromBackground {
                setupVideoFor(url: videoCellURL,index: videoCell.index!)
            }
            playVideo(withLayer: videoCell.videoLayer, url: videoCellURL,index: videoCell.index!)
        }
    }
    
    func pausePlayeVideos(tableView: UITableView, appEnteredFromBackground: Bool = false) {
        let visisbleCells = tableView.visibleCells
        var videoCellContainer: ASAutoPlayVideoLayerContainer?
        var maxHeight: CGFloat = 0.0
        for cellView in visisbleCells {
