...
class ProgressIndicator: UILabel {
...
//            }
    ...
    var isSelected: Bool = false {
        didSet {
            if isSelected {
                font = UIFont(name: "SegoeUI-Bold", size: 18)
                textColor = .white
                
            } else {
                font = UIFont(name: "SegoeUI-Light", size: 18)
                textColor = UIColor.colorGray2.withAlphaComponent(0.37)
            }
        }
    }
}

class MediaPollPostCell: UITableViewCell {
    
    static let reuseIdentifier = "MediaPollPostCell"
    ...
    @IBOutlet weak var vMediaContainer: UIView!
    ...
    @IBOutlet weak var imvPostTagInMedia: UIImageView! { didSet {
        imvPostTagInMedia.contentMode = .center
        imvPostTagInMedia.image = UIImage(named: "tag.poll")
        }}
    ...
    private func updateOptions(ForVote voted: Int, withPost post: PostModel) {
        // let's count from '1' as it's been increased by this new vote
        var totalVotes = 0
    ...
    func configureCell(_ post: PostModel, in page: Int = 0) {
        ...
        // show badge for only business user
        vBadgeContainer.isHidden = !post.isBusinessPost
    ...
    class func cellHeight(_ post: PostModel, isProfile: Bool = false) -> CGFloat {
        ...
        height += (SCREEN_WIDTH - 32)*179/170.0 // design ratio
        
        // add post container height
        // title top margin
        height += 10
        // poll questions
        let estimatedFrame = post.Post_Title.heightForString(CONTENT_PREFERRED_WIDTH, font: UIFont(name: Font.SegoeUISemibold, size: 15))
        let lineHeight = UIFont(name: Font.SegoeUISemibold, size: 15)!.lineHeight
        // limit 2 lines
...
// Use this protocol where you want to get option updated
protocol PollOptionSelectionDelegate {
    func optionSelected(_ selected: Int, completed: @escaping (Bool) -> Void)
}
